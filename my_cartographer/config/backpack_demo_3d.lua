...
options = {
  ...
  --use_pose_extrapolator=true, --追記
  use_odometry = false,
  use_nav_sat = false,
  use_landmarks = false,
  num_laser_scans = 0,--本来は2
...
TRAJECTORY_BUILDER_3D.min_range = 1.0 --最少計測径
TRAJECTORY_BUILDER_3D.max_range = 30 --30  --最大計測径
TRAJECTORY_BUILDER_3D.num_accumulated_range_data = 160--1 -- 160大切 --追記(urdfに依存?) TRAJECTORY_BUILDER_3D.voxel_filter_size = 0.05
--TRAJECTORY_BUILDER_3D.imu_gravity_time_constant = 9.81
--TRAJECTORY_BUILDER_3D.num_odometry_states = 1

TRAJECTORY_BUILDER_3D.high_resolution_adaptive_voxel_filter.max_length = 5--10　--追記
...
--TRAJECTORY_BUILDER_3D.use_online_correlative_scan_matching = true --追加
...
TRAJECTORY_BUILDER_3D.ceres_scan_matcher.rotation_weight = 4 --Default=4e2, Recommended=2e1
...
TRAJECTORY_BUILDER_3D.submaps.num_range_data = 80--100 --80--追記
...
POSE_GRAPH.optimize_every_n_nodes = 320 --100 -- 160,320 --11/28
...
POSE_GRAPH.constraint_builder.sampling_ratio = 0.03 --0.005 --0.03 --11/28
...
POSE_GRAPH.optimization_problem.huber_scale = 5e2 --3e2 --5e2	--11/28
...
POSE_GRAPH.optimization_problem.ceres_solver_options.max_num_iterations = 10 --200 --11/28
