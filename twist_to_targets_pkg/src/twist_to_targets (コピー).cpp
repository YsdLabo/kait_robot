...
#define MAX_VELOCITY        0.5            // ロボットの最高速度
...
#define MAX_STEERING_ANGULAR  1.2 // M_PI        // 操舵軸の最高回転速度
...
/*void finishedSteering(const std_msgs::Int32MultiArray& survo)
{
	for(int i=1;i<=4;i++)
		ROS_INFO("survo[%d]:%d",i,survo.data[i-1]);
	if(survo.data[0] == 1)
		finished_steering = 1;
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher steering_pub = nh.advertise<std_msgs::String>("cmd_msg",10);
    ros::Subscriber cmd_sub = nh.subscribe("/kait_robot/cmd_vel", 10, &cmdCallback);
    ros::Subscriber output_sub = nh.subscribe("/kait_robot/output", 10, &outputCallback);
